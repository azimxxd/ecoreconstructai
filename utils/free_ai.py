...
def _findings_cached(audit_json: str, address: str) -> dict:
    ...
    system = (
        ...
        "(YOLOv8 + OpenCV) по фотографии улицы. На их основе сформулируй выводы. "
        "Отвечай строго в JSON без markdown-обёртки, на русском языке."
    )
    user = (
        f"Адрес: {address}\n"
...
def _appeal_cached(
    ...
    user = (
        ...
        "EcoReconstruct AI. Верни только текст обращения без пояснений."
    )
    text = _chat(
        [{"role": "system", "content": system}, {"role": "user", "content": user}],
        max_tokens=900,
